<template>
  <dl class="s-item">
    ...
    <dd>
      ...
      <p>
        <el-rate v-model="meta.score" disabled show-score text-color="#ff9900"></el-rate>
        ...
        <span class="s-item-comment-total">{{meta.commentNum}}人点评</span>
      </p>
      <p>
      ...
      <ul class="deal-items">
        <li class="items" v-for="(item,index) in meta.deal_items.slice(0,2)" :key="index">
          <p class="deal-title">{{item.titel}}</p>
          <p>
